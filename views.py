...
def index(request):
    ...
    items = list(Post.published.all())
    # change 3 to how many random items you want
    # random_items = random.sample(items, 3)
    # print(random_items)
    # # if you want only a single random item
    random_item = random.choice(items)
    return render(request, "blog/index.html", {"last_post": last_post,'random_post':random_item})
...
#             else:
#                 return HttpResponse("شما نتونستید لاگین شوید")
#     else:
#         form=LoginForm()
...
def user_information(request,user_id):
    ...
    return render(request, "blog/user-information.html", context)
